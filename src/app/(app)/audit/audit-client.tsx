...
const PURPOSE_META: Record<AuditPurpose, { label: string; icon: string; color: string }> = {
  my_business: { label: 'My Business', icon: '🏢', color: 'bg-slate-900/40 text-blue-300 border-slate-700/40' },
  client:      { label: 'Client',      icon: '🤝', color: 'bg-blue-900/40 text-blue-300 border-blue-700/40' },
  competitor:  { label: 'Competitor',  icon: '🔍', color: 'bg-slate-900/40 text-slate-400 border-slate-600/40' },
...
const SEVERITY_META = {
  ...
  warning:  { label: 'Warning',  color: 'text-slate-500', bg: 'bg-slate-950/40 border-slate-700/40', dot: 'bg-slate-500' },
...
function ScoreRing({ score, size = 120 }: { score: number; size?: number }) {
  ...
  const color = score >= 70 ? '#2563eb' : score >= 50 ? '#64748b' : '#ef4444'
...
function MiniScoreCard({ label, score }: { label: string; score: number }) {
  const color = score >= 70 ? 'text-blue-400' : score >= 50 ? 'text-slate-500' : 'text-red-400'
  const bar   = score >= 70 ? 'bg-blue-600' : score >= 50 ? 'bg-slate-600' : 'bg-red-500'
  return (
...
function GeoCheckItem({ item, passed }: { item: typeof GEO_ITEMS[number]; passed: boolean }) {
  return (
    <div className={`flex items-center gap-2.5 p-2.5 rounded-lg border transition-all ${
      passed ? 'bg-slate-900/20 border-slate-700/40' : 'bg-slate-800/40 border-slate-700/40'
    }`}>
      <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-[11px] font-bold ${
        passed ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-500'
      }`}>
        {passed ? '✓' : '✗'}
      </div>
      <div className="min-w-0">
        <p className={`text-xs font-mono font-semibold truncate ${passed ? 'text-blue-300' : 'text-slate-400'}`}>
          {item.label}
        </p>
...
export default function AuditClient({ audits: initialAudits, websiteUrl, businessName, currentHealthScore }: AuditClientProps) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-5">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-3 text-sm">
              <div className="text-center">
                <div className={`text-xl font-extrabold ${selectedAudit.score >= 70 ? 'text-blue-400' : selectedAudit.score >= 50 ? 'text-slate-500' : 'text-red-400'}`}>
                  {selectedAudit.score}
                </div>
              ...
              {geo && (
                <>
                  ...
                  <div className="text-center">
                    <div className={`text-xl font-extrabold ${geo.geo_score >= 65 ? 'text-blue-400' : geo.geo_score >= 35 ? 'text-slate-500' : 'text-red-400'}`}>
                      {geo.geo_score}
                    </div>
                  ...
                  <div className={`px-2.5 py-1 rounded-full text-[11px] font-bold border ${
                    geo.ai_discoverability === 'high'   ? 'bg-slate-900/30 text-blue-400 border-slate-700/40' :
                    geo.ai_discoverability === 'medium' ? 'bg-slate-900/30 text-slate-500 border-slate-700/40' :
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-2 rounded-lg transition-colors whitespace-nowrap"
      ...
      {/* ── Warning Banner ── */}
      <div className="bg-slate-100 border-2 border-slate-400 rounded-2xl p-4 flex gap-3">
        <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-slate-500 flex items-center justify-center text-xl">
          ⚠️
        </div>
        <div>
          <p className="text-slate-800 font-bold text-sm">Enter YOUR business website only</p>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 bg-slate-800/60 border border-slate-700 focus:border-blue-500 focus:outline-none rounded-xl px-4 py-2.5 text-white text-sm placeholder:text-slate-600 transition-colors"
          />
          <button onClick={runAudit} disabled={running || !urlInput.trim()}
            className="bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-colors whitespace-nowrap flex items-center gap-2">
        ...
        {!websiteUrl && !running && urlInput.trim() && (
          <p className="text-xs text-slate-600 mt-2 flex items-center gap-1.5">
            <span>⚠</span> This URL won&apos;t be saved to your profile. <Link href="/settings" className="underline hover:text-slate-500">Save it in Settings</Link> to pre-fill next time.
          </p>
        )}
        {(running || logEntries.length > 0) && (
          <div className="mt-3 rounded-xl overflow-hidden border border-slate-700 bg-[#0d1117]">
            ...
            <div className="flex items-center gap-2 px-3 py-2 bg-slate-800/80 border-b border-slate-700">
              ...
              <span className="w-2.5 h-2.5 rounded-full bg-blue-600/70" />
              <span className="text-slate-500 text-[11px] font-mono ml-2">CooVex AI Engine — audit activity</span>
              {running && <span className="ml-auto flex items-center gap-1.5 text-[11px] text-blue-400 font-mono"><span className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" />running</span>}
              {!running && logEntries.some(e => e.type === 'done') && <span className="ml-auto text-[11px] text-blue-400 font-mono">● done</span>}
            </div>
            {/* Log body */}
              ...
              {logEntries.map(entry => (
                <div key={entry.id} className="flex items-start gap-2.5 animate-[fadeSlideIn_0.2s_ease]">
                  ...
                  <span className={
                    ...
                    entry.type === 'success' ? 'text-blue-500' :
                    entry.type === 'warn'    ? 'text-red-400' :
                    entry.type === 'sub'     ? 'text-slate-500 pl-2' :
                    'text-slate-300'
                  }>
                    {entry.type === 'info' && <span className="text-blue-400 mr-1.5">›</span>}
                    {entry.type === 'success' && <span className="text-blue-500 mr-1.5">✓</span>}
              ...
              return (
                <div key={audit.id} onClick={() => { setSelectedAudit(audit); setActiveTab('overview') }}
                  className={`cursor-pointer bg-slate-900 border rounded-xl p-4 transition-all group relative ${
                    isSelected ? 'border-blue-600/50 ring-1 ring-blue-600/20' : 'border-slate-800 hover:border-slate-700'
                  }`}>
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className={`text-base font-extrabold ${audit.score >= 70 ? 'text-blue-400' : audit.score >= 50 ? 'text-slate-500' : 'text-red-400'}`}>
                        {audit.score}
                      </span>
                      <span className="text-slate-600 text-xs">/100</span>
                      {auditGeo && (
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold border ${
                          auditGeo.ai_discoverability === 'high'   ? 'text-blue-400 bg-slate-900/30 border-slate-700/40' :
                          auditGeo.ai_discoverability === 'medium' ? 'text-slate-500 bg-slate-900/30 border-slate-700/40' :
                    ...
                    <button onClick={() => setActiveTab('geo')}
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            <div className={`text-2xl font-extrabold ${geo.geo_score >= 65 ? 'text-blue-400' : geo.geo_score >= 35 ? 'text-slate-500' : 'text-red-400'}`}>
                              {geo.geo_score}
                            </div>
                      ...
                      <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all ${
                          geo.geo_score >= 65 ? 'bg-blue-600' : geo.geo_score >= 35 ? 'bg-slate-600' : 'bg-red-500'
                    ...
                    <div className="space-y-3">
                      {(selectedAudit.report_json.recommendations || []).map((rec, i) => (
                        <div key={i} className="flex items-start gap-3 group">
                          <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-600/20 border border-blue-500/30 flex items-center justify-center text-blue-400 text-xs font-bold">
                            {i + 1}
                          </span>
                ...
                <div className="space-y-4">
                        ...
                        {(geo.ai_tasks || []).map((task, i) => {
                          ...
                          return (
                            <div key={i} className={`border rounded-xl overflow-hidden ${
                              ...
                              task.priority === 'high'     ? 'border-slate-700/40' :
                              'border-slate-700/40'
                            }`}>
                              <div className={`p-4 ${
                                task.priority === 'critical' ? 'bg-red-950/20' :
                                task.priority === 'high'     ? 'bg-slate-950/20' :
                                'bg-slate-800/40'
                              }`}>
                                <div className="flex items-start gap-3">
                                  <div className={`flex-shrink-0 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase mt-0.5 ${
                                    ...
                                    task.priority === 'high'     ? 'bg-slate-900/60 text-slate-400' :
                                  ...
                                  <button
                                    ...
                                    className="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all bg-blue-600 hover:bg-blue-500 text-white disabled:opacity-60"
                                  >
                  ...
                  {visibility ? (
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex items-center gap-3">
                      <span className="w-4 h-4 border-2 border-blue-400/30 border-t-violet-400 rounded-full animate-spin flex-shrink-0" />
                      <div>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                          ✨ Live AI Visibility Check
                          <span className="text-[10px] font-normal px-1.5 py-0.5 rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/25">Gemini Search</span>
                        </h3>
                      </div>
                        ...
                        {SEARCH_PRESENCE_ITEMS.map(item => {
                          ...
                          return (
                            <div key={item.key} className={`flex items-start gap-2.5 p-2.5 rounded-lg border transition-all ${
                              ...
                              <div className="min-w-0">
                                <p className={`text-xs font-semibold truncate ${passed ? 'text-blue-300' : 'text-slate-400'}`}>
                    ...
                    <div className="space-y-2 text-sm">
                      {[
                        ...
                        <div key={i} className={`flex items-start gap-3 p-3 rounded-xl border ${
                          row.ok ? 'bg-slate-900/10 border-slate-700/30' : 'bg-slate-800/60 border-slate-700/30'
                        }`}>
                          <span className={`text-sm flex-shrink-0 mt-0.5 ${row.ok ? 'text-blue-400' : 'text-red-400'}`}>
                            {row.ok ? '✓' : '✗'}
                          </span>
                          <div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-5 shadow-sm">
                            ...
                            return (
                              <a key={i} href={p} target="_blank" rel="noopener noreferrer"
                                className={`text-[11px] px-2 py-0.5 rounded-md border truncate max-w-[180px] ${
                                  isJs ? 'text-slate-700 bg-slate-100 border-slate-300' : 'text-blue-700 bg-blue-50 border-blue-200 hover:text-blue-900'
                    ...
                    {intel.description && (
                      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                        <p className="text-blue-900 text-sm leading-relaxed">{intel.description}</p>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {intel.clients!.map((c, i) => {
                            const palette = ['bg-blue-500','bg-blue-600','bg-blue-500','bg-slate-600','bg-rose-500','bg-blue-500']
                    ...
                    {(intel.missing_elements?.length ?? 0) > 0 && (
                      <div className="border-t border-slate-200 pt-4">
                        ...
                        {intel.missing_elements!.map((m, i) => (
                          <div key={i} className="flex items-center gap-2 text-sm text-slate-700 py-1"><span className="text-slate-600">⚠</span> {m}</div>
                        ))}
                      </div>
                    )}
...
function generateReportHtml(audit: Audit, businessName: string): string {
  ...
  const sc = (s: number) => s >= 70 ? '#2563eb' : s >= 50 ? '#64748b' : '#ef4444'
...
.gi-dot.p{background:#2563eb;color:#fff}
...
.rec-n{width:24px;height:24px;border-radius:50%;background:#2563eb;color:#fff;font-size:11px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0}
...
.intel-desc em{color:#2563eb;font-size:12px;display:block;margin-top:6px}
...
.ins-arrow{color:#2563eb;margin-right:8px;flex-shrink:0}
...
<div class="hdr">
  <div class="hdr-top">
    <div>
      ...
      <div style="font-size:12px;color:#93c5fd;margin-top:3px">Business Audit Report</div>
    </div>
    <div class="meta">
...
${geo ? `<div class="sec">
  ...
  <div style="margin-top:14px;padding-top:14px;border-top:1px solid #f1f5f9">
    ...
    ${[
      ...
      <div class="ai-row-icon" style="color:${row.ok ? '#2563eb' : '#ef4444'}">${row.ok ? '✓' : '✗'}</div>
    </div>`).join('')}
  </div>
</div>` : ''}
