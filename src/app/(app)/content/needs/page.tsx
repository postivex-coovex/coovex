...
export const metadata: Metadata = { title: 'Need To Create — CooVex' }

export default async function ContentNeedsPage() {
  ...
  const sc = biz?.social_connections as Record<string, { connected: boolean }> | null
  ...
  if (geoMemory?.value_text) {
    try { contentGaps = JSON.parse(geoMemory.value_text)?.content_gaps ?? [] } catch {}
  }
  ...
  return (
    <Suspense>
      <ContentClient
        ...
        hasWebhook={!!biz?.webhook_url}
        contentGaps={contentGaps}
        initialFilter="need_to_create"
      />
    </Suspense>
  )
}
