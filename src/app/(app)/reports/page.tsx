...
function starsBar(rating: number): string {
  ...
  return `<div style="display:flex;align-items:center;gap:8px;">
    ...
    <span style="font-size:12px;color:#f59e0b;font-weight:600;">${rating.toFixed(1)}★</span>
  </div>`
}
...
function metricCardsHtml(metrics: ComparisonMetric[], period: ReportPayload['period']): string {
  return `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;margin-bottom:32px;">
    ${metrics.map(m => {
      ...
      const color   = m.trend === 'up' ? '#10b981' : m.trend === 'down' ? '#ef4444' : '#64748b'
...
<style>
  * { box-sizing: border-box; }
  ...
  .improved { border-left: 4px solid #10b981; }
...
<div class="header">
  ...
  ${healthMetric ? `<div style="margin-top:16px;display:flex;align-items:center;gap:12px;">
    ...
    <div style="flex:1;height:8px;background:rgba(255,255,255,0.2);border-radius:4px;overflow:hidden;max-width:200px;">
      <div style="width:${healthMetric.curr}%;height:100%;background:#a78bfa;border-radius:4px;"></div>
    </div>
...
<!-- AI NARRATIVE -->
${aiSection('✅', '#10b981', 'What\'s Working', report.ai.winning, '#10b981')}
${aiSection('⚠️', '#f59e0b', 'Needs Attention', report.ai.needs_attention, '#ef4444')}

<!-- NEXT ACTIONS -->
<div class="section">
  <h2>Next Actions</h2>
  <div style="border-left:4px solid #7c3aed;padding:16px 20px;border-radius:0 8px 8px 0;background:#f5f3ff;">
...
<!-- FOOTER -->
<div class="footer">
  Generated by <strong style="color:#7c3aed;">CooVex</strong> AI Business Agent &nbsp;·&nbsp; ${dateStr}
</div>
        ...
        <div className="mb-6 bg-slate-900 border border-slate-800 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              { label: 'Awaiting', value: proposalStats.sent,     color: 'text-amber-400',     dot: 'bg-amber-400' },
              { label: 'Accepted', value: proposalStats.accepted, color: 'text-emerald-400',   dot: 'bg-emerald-400' },
          ...
          {proposalStats.total > 0 && (
            <div className="mt-3 h-1.5 bg-slate-800 rounded-full overflow-hidden flex">
              <div className="bg-amber-400 h-full transition-all" style={{ width: `${(proposalStats.sent / proposalStats.total) * 100}%` }} />
              <div className="bg-emerald-400 h-full transition-all" style={{ width: `${(proposalStats.accepted / proposalStats.total) * 100}%` }} />
          ...
          <div key={t.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-violet-600/20 hover:bg-violet-600/30 text-violet-400 text-xs font-medium rounded-lg border border-violet-800/40 transition-colors disabled:opacity-50"
              >
                {generating === t.id
                ...
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-colors ${
                    templates.find(t => t.id === scheduleModal)?.schedule === s
                      ? 'bg-violet-600/20 border-violet-700/40 text-white'
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-sm">
            ...
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-violet-500 transition-colors mb-4"
            />
            <p className="text-amber-400 text-xs mb-4">Email delivery requires Resend setup. Share the portal link as an alternative.</p>
            <div className="flex gap-2">
              ...
              <button onClick={() => setClientEmail('')} className="flex-1 py-2 bg-violet-600 hover:bg-violet-500 text-white text-sm font-medium rounded-xl transition-colors">Send</button>
            </div>
          </div>
        </div>
      )}
