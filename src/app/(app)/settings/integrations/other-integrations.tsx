...
function IntegrationCard({ tool, saved }: { tool: typeof TOOLS[0]; saved: IntegrationSettings | undefined }) {
  ...
  return (
    <div className={`bg-slate-900 border rounded-2xl overflow-hidden transition-colors ${
      isEnabled ? 'border-slate-700/40' : 'border-slate-800'
    }`}>
      {/* Header */}
      <div
      ...
      >
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl flex-shrink-0 ${
          isEnabled ? 'bg-slate-950/40 border border-slate-700/40' : 'bg-slate-800'
        }`}>
          {tool.icon}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {tool.badge && (
              <span className="text-[10px] px-1.5 py-0.5 bg-slate-950/50 text-blue-400 border border-slate-700/40 rounded-full">{tool.badge}</span>
            )}
            {tool.syncRevenue && (
              <span className="text-[10px] px-1.5 py-0.5 bg-slate-950/50 text-blue-400 border border-slate-700/40 rounded-full">Syncs Revenue</span>
            )}
            {tool.oauthOnly && (
              <span className="text-[10px] px-1.5 py-0.5 bg-slate-950/50 text-slate-500 border border-slate-700/40 rounded-full">OAuth — Coming Soon</span>
            )}
            {isEnabled && !tool.oauthOnly && (
              <span className="text-[10px] px-1.5 py-0.5 bg-slate-950/50 text-blue-400 border border-slate-700/40 rounded-full">Active</span>
            )}
          </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {tool.syncRevenue && isEnabled && (
            <button
              ...
              disabled={syncing}
              className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-slate-900/40 hover:bg-slate-900/60 disabled:opacity-50 text-blue-300 border border-slate-700/40 rounded-lg transition-colors"
            >
        ...
        <div className="px-5 pb-5 border-t border-slate-800/60 pt-4">
          <div className="flex items-start justify-between gap-3 mb-3">
            ...
            <Link
              ...
              className="flex-shrink-0 flex items-center gap-1.5 bg-blue-700/70 hover:bg-blue-600 text-white text-xs font-semibold px-3 py-2 rounded-lg transition-colors whitespace-nowrap"
            >
              🤖 Ask AI
            </Link>
          </div>
          <div className="bg-slate-950/20 border border-slate-700/30 rounded-xl p-3">
            <p className="text-slate-500 text-xs font-medium">OAuth Connection Required</p>
      ...
      {!tool.oauthOnly && open && (
        <div className="px-5 pb-5 border-t border-slate-800/60 pt-4 space-y-4">
          {/* Ask AI + benefit */}
          <div className="flex items-start justify-between gap-3 bg-slate-950/15 border border-slate-700/25 rounded-xl p-3">
            <p className="text-blue-300/80 text-xs leading-relaxed">{tool.desc}</p>
            <Link
          ...
          {tool.syncRevenue && (
            <div className="bg-slate-950/20 border border-slate-700/30 rounded-xl p-3">
              <p className="text-blue-400 text-xs font-medium">Revenue Sync Enabled</p>
          ...
          {tool.fields.map(f => (
            <div key={f.key}>
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2.5 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-blue-500 transition-colors font-mono"
              />
            </div>
          ))}

          {msg && (
            <p className={`text-sm px-3 py-2 rounded-lg ${msg.ok ? 'bg-slate-950/30 text-blue-400' : 'bg-red-950/30 text-red-400'}`}>
              {msg.ok ? '✓' : '✗'} {msg.text}
            </p>
          )}
