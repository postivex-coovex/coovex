...
const PLATFORM_COLOR = {
  ...
  content:   'bg-blue-600/20 text-blue-300 border-blue-500/30',
  outreach:  'bg-slate-600/20 text-slate-400 border-slate-500/30',
}
const STATUS_CONFIG = {
  not_started: { label: 'Not Started', color: 'bg-slate-700 text-slate-400 border-slate-600' },
  in_progress: { label: 'In Progress', color: 'bg-slate-600/20 text-slate-400 border-slate-500/40' },
  done:        { label: 'Done ✓',      color: 'bg-blue-600/20 text-blue-300 border-blue-500/40' },
}
...
const FEATURE: Record<string, { link: string; label: string; color: string }> = {
  'leads':       { link: '/leads',       label: 'Open AI Lead Finder',  color: 'bg-blue-600 hover:bg-blue-500' },
  'leads/cold':  { link: '/leads/cold',  label: 'Cold Outreach',        color: 'bg-blue-600 hover:bg-blue-500' },
  'content':     { link: '/content',     label: 'Write with AI',        color: 'bg-blue-600 hover:bg-blue-600' },
  'campaigns':   { link: '/campaigns',   label: 'Create Campaign',      color: 'bg-blue-600 hover:bg-blue-500' },
  'proposals':   { link: '/proposals',   label: 'Build Proposal',       color: 'bg-blue-600 hover:bg-blue-500' },
  'competitors': { link: '/competitors', label: 'Track Competitors',    color: 'bg-red-600 hover:bg-red-500' },
  'reviews':     { link: '/reviews',     label: 'Manage Reviews',       color: 'bg-slate-600 hover:bg-slate-600' },
...
const PHASE_ACCENT = ['border-blue-500/40','border-blue-500/40','border-blue-500/40','border-slate-500/40']
const PHASE_LABEL  = ['text-blue-400','text-blue-400','text-blue-400','text-slate-500']
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
      <div className="relative w-20 h-20 mx-auto mb-8">
        <div className="absolute inset-0 rounded-full border-2 border-blue-500/10 animate-ping" style={{ animationDuration: '2s' }} />
        <div className="absolute inset-1 rounded-full border-2 border-blue-500/20 animate-ping" style={{ animationDuration: '2.5s', animationDelay: '0.3s' }} />
        <div className="absolute inset-0 rounded-full border-2 border-blue-500/30 border-t-violet-500 animate-spin" style={{ animationDuration: '1.2s' }} />
      ...
      <div className="inline-flex items-center gap-2 bg-blue-600/20 border border-blue-500/30 rounded-full px-4 py-1.5 mb-5">
        <span className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" />
        <span className="text-blue-300 text-xs font-medium truncate max-w-xs">{goal}</span>
      </div>
      ...
      <div className="flex items-center justify-center gap-1.5">
        {LOADING_STEPS.map((_, i) => (
          <div key={i} className="rounded-full transition-all duration-500" style={{ width: i === step ? '20px' : '6px', height: '6px', background: i === step ? '#2563eb' : i < step ? '#4c1d95' : '#1e293b' }} />
        ))}
      </div>
    </div>
  )
...
function AssistPanel({ action, onClose, bizName, industry, target }: {
  ...
  return (
    <div className="mt-3 bg-slate-800/80 border border-blue-500/30 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <p className="text-blue-300 text-xs font-semibold">{AI_HELP_LABEL[action.ai_help_type ?? ''] ?? 'AI Help'}</p>
        <button onClick={onClose} className="text-slate-500 hover:text-slate-300 text-xs">✕ Close</button>
      </div>
      {!content ? (
        <button onClick={generate} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-xs font-medium py-2 rounded-lg transition-colors flex items-center justify-center gap-2">
        ...
        <div>
          ...
          <div className="flex gap-2 mt-2">
            ...
            <button onClick={generate} disabled={loading} className="text-xs text-blue-400 hover:text-blue-300 px-3 py-1.5 transition-colors">↺ Regenerate</button>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className={`bg-slate-900/60 border border-slate-700/50 rounded-xl p-4 transition-opacity ${done ? 'opacity-50' : ''}`}>
      <div className="flex items-start gap-2 mb-2">
        <button onClick={onToggle} className={`mt-0.5 w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center transition-colors ${done ? 'bg-blue-600 border-blue-600' : 'border-slate-600 hover:border-slate-400'}`}>
      ...
      <div className="flex gap-2 flex-wrap mt-3">
        ...
        {hasAiHelp && (
          <button onClick={() => setShowAssist(v => !v)} className="text-xs text-blue-400 hover:text-blue-300 border border-blue-500/30 hover:border-blue-400/50 px-3 py-1.5 rounded-lg transition-colors">
  ...
  return (
    <div>
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-slate-900 border border-slate-500/30 rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-slate-500">{inProgressCount}</p>
        ...
        <div className="bg-slate-900 border border-blue-500/30 rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-blue-400">{doneCount}</p>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 mb-6">
        ...
        <div className="h-2.5 bg-slate-800 rounded-full">
          <div className="h-2.5 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full transition-all duration-500" style={{ width: `${(doneCount / LAUNCH_PLATFORMS.length) * 100}%` }} />
        </div>
      ...
      <div className="space-y-3">
        {LAUNCH_PLATFORMS.map(platform => {
          ...
          return (
            <div key={platform.id} className={`bg-slate-900 border rounded-xl overflow-hidden transition-all ${state.status === 'done' ? 'border-blue-500/30' : state.status === 'in_progress' ? 'border-slate-500/30' : 'border-slate-800'}`}>
              ...
              {isExpanded && (
                <div className="px-4 pb-4 border-t border-slate-800">
                  <div className="bg-slate-900/20 border border-blue-500/20 rounded-lg p-3 mb-3 mt-3">
                    <p className="text-blue-300 text-xs leading-relaxed">💡 {platform.tip}</p>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                    <div>
                      <label className="text-slate-500 text-xs mb-1 block">URL / Link</label>
                      <input value={state.url} onChange={e => update(platform.id, 'url', e.target.value)} placeholder="https://…" className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-300 text-xs placeholder-slate-600 focus:outline-none focus:border-blue-500 transition-colors" />
                    </div>
                    <div>
                      <label className="text-slate-500 text-xs mb-1 block">Notes / Metrics</label>
                      <input value={state.notes} onChange={e => update(platform.id, 'notes', e.target.value)} placeholder="e.g. 240 upvotes, 12 signups…" className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-300 text-xs placeholder-slate-600 focus:outline-none focus:border-blue-500 transition-colors" />
                    </div>
                  </div>
                  <button onClick={() => setAiPanel(isAiOpen ? null : platform.id)} className={`text-xs font-medium px-4 py-2 rounded-lg border transition-colors ${isAiOpen ? 'bg-blue-600/20 border-blue-500/50 text-blue-300' : 'border-blue-500/30 text-blue-400 hover:border-blue-400/60 hover:text-blue-300'}`}>
                    {isAiOpen ? 'Hide AI Help' : (AI_HELP_LABEL_MAP[platform.id] ?? '✨ AI Help')}
                  </button>
                  {isAiOpen && (
                    <div className="mt-3 bg-slate-800/60 border border-blue-500/20 rounded-xl p-4">
                      <p className="text-blue-300 text-xs font-semibold mb-3">{AI_HELP_LABEL_MAP[platform.id] ?? 'AI Content'} for {platform.name}</p>
                      {!content ? (
                        <button onClick={() => generateAiHelp(platform.id)} disabled={isGenerating} className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-sm font-medium py-2.5 rounded-lg transition-colors flex items-center justify-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button onClick={() => generateAiHelp(platform.id)} disabled={isGenerating} className="text-xs text-blue-400 hover:text-blue-300 px-3 py-1.5 transition-colors">↺ Regenerate</button>
                          </div>
                        </div>
                      )}
                    </div>
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
        ...
        <button onClick={discover} disabled={loading} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-sm font-medium px-4 py-2 rounded-xl transition-colors flex-shrink-0">
      ...
      {communities.length > 0 && (
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[['Communities', communities.length, 'text-white', 'border-slate-800'], ['Joined', joinedCount, 'text-blue-400', 'border-blue-500/30'], ['Posted', postedCount, 'text-blue-400', 'border-blue-500/30']].map(([label, count, color, border]) => (
      ...
      {communities.length === 0 && !loading && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
          ...
          <button onClick={discover} className="bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium px-6 py-2.5 rounded-xl transition-colors">🔍 Discover Communities</button>
        </div>
      )}
      {communities.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {communities.map((c, i) => {
            ...
            return (
              <div key={i} className={`bg-slate-900 border rounded-xl p-4 transition-all ${state.joined ? 'border-blue-500/30' : 'border-slate-800'}`}>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <button onClick={() => toggleJoined(c.name)} className={`text-xs font-medium px-3 py-1.5 rounded-lg border transition-colors ${state.joined ? 'bg-blue-600/20 text-blue-300 border-blue-500/40' : 'bg-slate-800 text-slate-400 border-slate-700 hover:border-slate-600'}`}>{state.joined ? '✓ Joined' : '+ Join'}</button>
                  <button onClick={() => togglePosted(c.name)} className={`text-xs font-medium px-3 py-1.5 rounded-lg border transition-colors ${state.posted ? 'bg-blue-500/20 text-blue-300 border-blue-500/40' : 'bg-slate-800 text-slate-400 border-slate-700 hover:border-slate-600'}`}>{state.posted ? '✓ Posted' : '📝 Mark Posted'}</button>
                  ...
                  <button onClick={() => setPostPanel(isPostOpen ? null : c.name)} className="ml-auto text-xs text-blue-400 hover:text-blue-300 border border-blue-500/30 hover:border-blue-400/50 px-3 py-1.5 rounded-lg transition-colors">✨ Write Post</button>
                </div>
                {isPostOpen && (
                  <div className="mt-3 bg-slate-800/60 border border-blue-500/20 rounded-xl p-3">
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-blue-300 text-xs font-semibold">AI Post for {c.name}</p>
                      <button onClick={() => setPostPanel(null)} className="text-slate-500 hover:text-slate-300 text-xs">✕</button>
                    </div>
                    {!posts[c.name] ? (
                      <button onClick={() => generatePost(c)} disabled={generating === c.name} className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-xs font-medium py-2 rounded-lg transition-colors flex items-center justify-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => generatePost(c)} disabled={generating === c.name} className="text-xs text-blue-400 hover:text-blue-300 px-3 py-1.5 transition-colors">↺ Regenerate</button>
                        </div>
                      </div>
                    )}
                  </div>
  ...
  function buildPdf(): string {
    ...
    const phasesHtml = plan.phases.map(ph => `
      <div style="margin-bottom:32px;page-break-inside:avoid">
        <h2 style="font-size:18px;color:#2563eb;margin-bottom:4px">${ph.name} — ${ph.weeks}</h2>
        ...
        ${ph.actions.map(a => `
          <div style="border:1px solid #e2e8f0;border-radius:8px;padding:14px;margin-bottom:10px">
            ...
            ${FEATURE[a.action_type] ? `<p style="font-size:12px;color:#2563eb;margin-top:6px">→ CooVex: ${FEATURE[a.action_type].label}</p>` : ''}
          </div>`).join('')}
      </div>`).join('')
...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:32px">
  ${plan.expected_results.map(r => `<div style="border:1px solid #e2e8f0;border-radius:8px;padding:12px"><p style="margin:0 0 4px;font-size:11px;color:#94a3b8">${r.label}</p><p style="margin:0;font-size:20px;font-weight:700;color:#2563eb">${r.value}</p></div>`).join('')}
</div>
${phasesHtml}
</body></html>`
  }
    ...
    <>
      <div className="p-6 max-w-7xl mx-auto">
        <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
          ...
          {activeTab === 'plan' && plan && (
            <button onClick={() => setExportHtml(buildPdf())} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium px-4 py-2 rounded-xl transition-colors flex-shrink-0">
              📄 Export PDF
            </button>
          )}
        </div>
        ...
        <div className="flex gap-1 bg-slate-900 border border-slate-800 rounded-xl p-1 mb-6 w-fit">
          {TABS.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`text-sm font-medium px-4 py-2 rounded-lg transition-all ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}>{tab.label}</button>
          ))}
        </div>
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 mb-4">
                  {GOAL_PRESETS.map(g => {
                    ...
                    return (
                      <button key={g.label} onClick={() => selectGoal(g.label)} className={`relative text-left rounded-xl p-4 transition-all border ${isActive ? 'bg-blue-600/20 border-blue-500 shadow-[0_0_0_1px_#2563eb]' : 'bg-slate-900 hover:bg-slate-800 border-slate-800 hover:border-blue-500/40'}`}>
                        ...
                        {isSaved && !isActive && <span className="absolute top-2 right-2 text-[9px] bg-blue-600/20 text-blue-400 border border-blue-500/30 px-1.5 py-0.5 rounded">saved</span>}
                      </button>
                    )
                  })}
                </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-6">
                  <div className="flex gap-3">
                    ...
                    <button onClick={() => selectGoal(customGoal)} disabled={!customGoal.trim()} className="bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white text-sm font-medium px-5 py-2.5 rounded-lg transition-colors flex-shrink-0">✨ Build Plan</button>
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                      {plan.expected_results.map((r, i) => (
                        <div key={i} className="bg-slate-900 border border-slate-800 rounded-xl p-3 text-center">
                          ...
                          <p className="text-blue-400 font-bold text-lg">{r.value}</p>
                        </div>
                      ))}
                    </div>
      ...
      {exportHtml && (
        <div className="fixed inset-0 z-[60] flex flex-col bg-black">
          <div className="flex items-center justify-between px-5 py-3 bg-slate-900 border-b border-slate-800 flex-shrink-0">
            ...
            <div className="flex gap-2">
              <button onClick={() => iframeRef.current?.contentWindow?.print()} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white text-sm px-4 py-1.5 rounded-lg transition-colors">🖨️ Print / Save PDF</button>
